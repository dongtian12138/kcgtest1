...
> 快照时间：2026-08-24T09:32:26Z
> 当前分支：`carts-grasp-v2-rebuild-20260823`
> 本文件只做恢复路由；物理结论以 V2 原始运行产物为准。

## 一句话状态

run15 把对象场景 `f1j3` 尖峰从 48.013 降到 4.888 rad/s，但仍触发 3 rad/s 急停；当前只允许同一根因的第二次、也是最后一次持续实测位置保持修复。

## 六行恢复摘要

- 总目标：真实允许表面候选 → 任务载荷鲁棒 Top-3 → Isaac 三指接触、离桌、抬升 50 mm、保持至少 2 s；两个对象同算法同主要参数。
- 当前里程碑：`V2_RECOVERY_2_OBJECT_A_CONTINUOUS_COMPLIANT_HOLD_FIX`；对象 B 关闭，只允许最后一次同根因修复、预检和 A 验证。
- 已经完成：run15 的一次性实测位置保持使 `f1j3` 峰值从 48.013 降到 4.888 rad/s、最大手部受力从 0.5816 降到 0.3190 N·m，但仍急停；三指、抬升和保持均未完成。
- 当前物理/算法阻塞：已停止指在其他手指闭合时继续固定同一目标，实际回弹后重新积累位置弹簧并激发从动关节。
- 下一步最简单动作：预紧前每周期把已停止指目标更新为自身实测位置；不改阈值、增益或力限。随后只做绑定预检和一次 A 验证，失败即停止该根因。
...
- V2 正式候选：空
- V2 研究动态门：`allowed=true`，scope=`OBJECT_A_FINAL_F1J3_FIX_VALIDATION_ONLY`；只允许最后修复、绑定预检和一次 A 验证，对象 B 关闭
