...
> 快照时间：2026-08-24T15:38:00Z
> 当前分支：`carts-grasp-graspgenx-route1-20260824`
> 本文件只做恢复路由；物理结论以原始运行产物为准。

## 一句话状态

GraspGenX-CARTS 已完成 5 个跨对象 KCG 手描述器和两个真实对象的米制网格绑定；官方预训练模型仍在隔离环境安装，尚无新六维候选、接触、离桌或抬升结果。

## 六行恢复摘要

- 最初目标：两个 D38999/J599 对象同一方法与主要参数，三根真实指腹接触、离桌、抬升至少 50 mm、保持至少 2 s，无未授权穿透。
- 当前已完成：从 `bd710ee` 创建并推送新分支；固定官方 commit `b942909`；生成 5 个对象无关描述器；两个登记网格的单位、尺寸与来源哈希已绑定。
- 当前真实物理结果：描述器相位 `0.50` 下 9 个预构型共检查 4203 个空手状态且无自碰；新路线尚未启动 Isaac，旧 A/B 结果只证明轴对称窄候选族失败。
- 当前唯一主要阻塞：先完成官方 checkpoint、GPU 推理和 `(K,4,4)` 六维位姿正对照；隔离 `uv sync` 仍有实质下载进展。
- 最近用户指令：`METHOD_REVISION + NEW_AUTONOMOUS_EXECUTION_WINDOW + UNATTENDED_EXECUTION`，不是硬件授权或降低成功标准。
- 下一步：官方内置三指手正对照 → 5 个固定 KCG 描述器对两个真实网格推理 → 六维覆盖 → V2 物理筛选与 Isaac。
