...
> 快照时间：2026-08-24T06:42:18Z
> 当前分支：`carts-grasp-v2-rebuild-20260823`
> 本文件只做恢复路由；物理结论以 V2 原始运行产物为准。

## 一句话状态

`CARTS-Grasp V2` 已完成双对象同配置 Top-3 和真实 Isaac 预飞，但没有完成三指闭合、离桌、抬升 50 mm 或保持 2 s；12 小时硬截止已经过去，A/B 动态保持 PARK，既有提交已同步到远端同名分支。

## 六行恢复摘要

- 总目标：真实允许表面候选 → 任务载荷鲁棒 Top-3 → Isaac 三指接触、离桌、抬升 50 mm、保持至少 2 s；两个对象同算法同主要参数。
- 当前里程碑：12 小时交付已停止新增功能；离线算法、动态失败证据、方法文档、最终回归和 Git 交付均已收口；对象动态 PARK。
...
- 下一步最简单动作：本轮停止；若用户以后明确重开动态研究，先做无对象无桌面手部同轨迹对照，不调参数，再决定是否允许对象运行。
- 绝对禁止跑偏方向：续修 H102、对象专用坐标/阈值、磁吸/隐藏固定、在线读取对象/接触/PhysX 真值、写物体位姿、把退出 0 冒充物理成功。

## 当前权威字段

- 状态：`PARKED`
- 当前里程碑：`V2_12H_DEADLINE_CLOSEOUT_DYNAMIC_NOT_ACHIEVED`
- V2 正式候选：空
- V2 研究动态门：`allowed=false`（12 小时硬截止已过；A/B 对象运行 PARK，孤立手部预形状对照未执行）
...
- Isaac 前：本轮硬截止已过，不再启动；若用户明确重开，先完成无对象/无桌面手部同轨迹对照，再核对精确进程、旧正式字段、V2 研究门、目标 run_id 和产物时间。
