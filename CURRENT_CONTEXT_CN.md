...
> 快照时间：2026-08-24T09:26:20Z
> 当前分支：`carts-grasp-v2-rebuild-20260823`
> 本文件只做恢复路由；物理结论以 V2 原始运行产物为准。

## 一句话状态

run13 在无对象、无桌面条件下逐周期复现 run12 全部 1104 个目标且没有速度尖峰，说明对象场景是异常的必要条件；当前只允许一次“接触确认后保持实测位置”的最小修复验证。

## 六行恢复摘要

- 总目标：真实允许表面候选 → 任务载荷鲁棒 Top-3 → Isaac 三指接触、离桌、抬升 50 mm、保持至少 2 s；两个对象同算法同主要参数。
- 当前里程碑：`V2_RECOVERY_2_OBJECT_A_CONTACT_STOP_FIX`；对象 B 关闭，只允许一次最小修复、绑定预检和 A 验证。
- 已经完成：run13 无对象/桌面回放 1104/1104 目标、最大目标差 0；`f1j3` 峰值 0.1588 rad/s、手部峰值 0.4352 rad/s，未复现 run12 的 48.013 rad/s。
- 当前物理/算法阻塞：对象场景交互是速度尖峰的必要条件；run12 接触确认后仍保持领先实际约 0.0266 rad 的位置目标，继续硬推可能触发接触—mimic 约束振荡。
- 下一步最简单动作：接触确认时把该指保持目标改为当周期实测位置；阈值、增益、力限和轨迹不变，随后重做绑定预检与一次 A 抓取。
...
- V2 正式候选：空
- V2 研究动态门：`allowed=true`，scope=`OBJECT_A_CONTACT_STOP_FIX_VALIDATION_ONLY`；只允许最小修复、绑定预检和一次 A 验证，对象 B 关闭
