...
> 快照时间：2026-08-21T15:05:33Z
> 用途：新对话、上下文压缩恢复和普通源码工作先读本文件，避免默认加载完整历史台账。
> 边界：本文件是当前状态的轻量路由，不覆盖追加式历史证据；出现冲突、时间漂移或准备动态运行时必须回查原始控制文件和运行产物。

## 一句话状态

当前研究主线是 `CARTS-GRASP-CROSS-OBJECT-V1`：三指接触范围计划已进入顶层生成器、断点、部分手部连续防碰撞和受力根域审计，当前完整静态回归 `455 passed`。但检查范围还不包括实体包含关系、权威整机/环境全路径、区间接触雅可比和接触范围受力下界，非摩擦误差也未标定，因此没有正式候选且禁止启动 Isaac。
...
- 进程快照：2026-08-21T15:05:33Z 未发现活动 Isaac、目标 runner 或 pytest；动态前必须重新核对，不能沿用本快照。
- 最新 CARTS 完整静态回归：`455 passed in 416.35s`，测试前后受检文件 SHA-256 清单一致。

## 已有证据与不能声称的内容

- 任务控制面的最新持久快照仍记录 `420 passed in 410.94s`；其后本地检查点 `259de95` 在稳定工作树实测 `455 passed in 416.35s`。两者都只是静态程序证据，不升级为仿真或动态通过。
- study canonical SHA-256：`7b1c0e7a20a7404a24eacf1c5923f91f4af27e96c9bca6798d14a0ce697ed664`，`freeze_eligible=false`。
...
- 手部非PAD表面对对象和自碰撞现已能覆盖注册接触范围及两个独立闭合阶段的完整乘积；受力模块也会登记每指全部最早接触根及三指完整组合数量，不读取显示中间值。
- 碰撞与受力证书仍是 `NOT_CERTIFIABLE`：受力模块明确不计算裕度，仍缺区间接触雅可比、参数化受力下界、实体包含和权威整机/环境全路径。
...
1. `MISSING_FORMAL_ROOT_INTERVAL_CANDIDATE_PROPAGATION`
   - 接触范围计划已到达顶层、断点、部分手部防碰撞和受力根域审计；还需给出经证明的接触范围受力下界并进入正式排序，禁止使用显示用近似值。
...
- 第二轮非 CARTS 套件实测 `2494 passed in 100.42s`；冻结合同/安装树与方法合同定向测试 `176 passed`；最新 CARTS 完整套件 `455 passed in 416.35s`。均为静态证据。
