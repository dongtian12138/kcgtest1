...
- 下一步：本窗口已到硬截止，只完成定向复核、双监督、提交和事实报告；后续方法实验必须另行预登记，不自动延长本窗口。

## 当前权威事实

- 状态：`PARKED`
- 里程碑：`GRASPGENX_ROUTE1_12H_CLOSED_NO_RESEARCH_EXECUTABLE`
...
- 确定性重放：同环境、同种子、独立进程/临时目录重放后，A/B 的 `proposals` 数组逐字节一致；下游候选表、覆盖和场景审计也逐字节一致，结果 JSON 去除纯耗时字段后一致。这只证明离线可复现，不是统计独立实验。
