...
> 快照时间：2026-08-21T13:04:37Z
> 用途：新对话、上下文压缩恢复和普通源码工作先读本文件，避免默认加载完整历史台账。
> 边界：本文件是当前状态的轻量路由，不覆盖追加式历史证据；出现冲突、时间漂移或准备动态运行时必须回查原始控制文件和运行产物。

## 一句话状态

当前研究主线是 `CARTS-GRASP-CROSS-OBJECT-V1`：三指原始接触表面和抓法入口的证据传递已修正，清理后本地完整静态检查417项通过；尚未把三指接触范围形成正式抓法，也未完成整机防碰撞和非摩擦误差标定，因此没有正式候选且禁止启动 Isaac。
...
- 状态：`IMPLEMENTING`
- 当前前沿：`FORMAL_ROOT_INTERVAL_PROPAGATION_AND_COMPLETE_COLLISION_AND_NONFRICTION_UNCERTAINTY_CERTIFICATION`
...
- 工作树状态：当前存在大量用户和历史未提交文件，均须原样保留；清理后 CARTS-Grasp 已完成417项回归，本轮未修改这些在研文件，不得清理、暂存或回退无关变化。

## 已有证据与不能声称的内容

- 任务控制面最新持久静态快照记录 `414 passed in 392.53s`；本轮清理后的当前工作树另行实测 `417 passed in 411.08s`，退出码0。新增3项尚未写回控制面，因此两条记录不能混为同一持久快照。
...
- 当前 CARTS 范围仍为28个方法模块、2个 Isaac 接口和24个测试；本轮没有修改这些文件。新对话默认只读这些路径，不遍历其他历史/下游实现。
