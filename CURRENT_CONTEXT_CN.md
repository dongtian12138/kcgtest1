...
- 当前唯一主要阻塞：官方手对同一 A/B 完整点云可稳定输出 256 个有限位姿，而 KCG 长指描述器仍没有完整路径安全候选；其尺度超出已审计官方描述器清单只是待验证原因之一，不能据此断言训练域或唯一根因。
- 最近用户指令：`METHOD_REVISION + NEW_AUTONOMOUS_EXECUTION_WINDOW + UNATTENDED_EXECUTION`，不是硬件授权或降低成功标准。
- 下一步：提交并 push 复现证据，再在剩余窗口只做最终完整性复核；不再看结果调整工作区箱、偏移或面标签。

## 当前权威字段

- 状态：`IMPLEMENTING`
- 当前里程碑：`GRASPGENX_ROUTE1_REFERENCE_RUNTIME_REPRODUCIBILITY_CONFIRMED`
...
- 已审计描述器尺度对照：KCG open 工作区 Y 为 `0.1175–0.3055 m`，已审计官方描述器清单最大约 `0.0673 m`；每对象 640 条保留提案中学习分支仅 26 条，其余 614 条来自官方 OBB 分支。该对照不代表完整训练分布，也不证明失败因果。
