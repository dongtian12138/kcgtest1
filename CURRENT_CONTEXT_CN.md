...
> 快照时间：2026-08-23T14:56:17Z
> 当前分支：`carts-grasp-v2-rebuild-20260823`
> 本文件只做恢复路由；物理结论以 V2 原始运行产物为准。

## 一句话状态

`CARTS-Grasp V2` 已冻结旧 H102；Isaac 6.0.1 最小场景、iiwa＋三指手 articulation、现有 D38999 桌面代理回归均已推进物理时间，但尚未产生 V2 候选，也没有真实 CAD 抓取动态成功结论。

## 六行恢复摘要

- 总目标：真实允许表面候选 → 任务载荷鲁棒 Top-3 → Isaac 三指接触、离桌、抬升 50 mm、保持至少 2 s；两个对象同算法同主要参数。
- 当前里程碑：V2 模型、真实允许面与候选生成。
- 已经完成：起始恢复点与首提交；双监督有条件批准；三层 Isaac 前置烟雾通过，包括 D38999 桌面代理自由落体和机器人 Home 保持。
- 当前物理/算法阻塞：真实 CAD 允许面唯一映射和 V2 离线管线尚未实现；代理场景不构成抓取证据。
- 下一步最简单动作：薄适配对象/手/允许面与公开力学接口，先做双对象候选非空。
- 绝对禁止跑偏方向：续修 H102、对象专用坐标/阈值、磁吸/隐藏固定、在线读取对象/接触/PhysX 真值、写物体位姿、把退出 0 冒充物理成功。

## 当前权威字段

- 状态：`IMPLEMENTING`
- 当前里程碑：`V2_MODEL_AND_CANDIDATE_GENERATION`
- V2 正式候选：空
...
- Isaac：6.0.1.0；最小场景、15 DOF 机器人和现有 D38999 桌面代理回归已运行；真实 CAD V2 抓取未运行
