...
> 快照时间：2026-08-23T14:54:40Z
> 当前分支：`carts-grasp-v2-rebuild-20260823`
> 本文件只做恢复路由；物理结论以 V2 原始运行产物为准。

## 一句话状态

`CARTS-Grasp V2` 已从提交 `330c54d` 建立独立分支并冻结旧 H102 路线；Isaac 6.0.1 最小场景与现有 iiwa＋三指手 articulation 烟雾已通过，但尚未加载活动连接器、尚未产生 V2 候选，也没有抓取动态成功结论。

## 六行恢复摘要

- 总目标：真实允许表面候选 → 任务载荷鲁棒 Top-3 → Isaac 三指接触、离桌、抬升 50 mm、保持至少 2 s；两个对象同算法同主要参数。
- 当前里程碑：活动连接器场景烟雾与 V2 模型适配。
- 已经完成：起始恢复点；双监督有条件批准；Isaac 最小物理 2.0 s 和 15 关节 iiwa＋三指手 articulation 烟雾通过。
- 当前物理/算法阻塞：活动连接器未加载；真实允许面唯一映射和 V2 离线管线尚未实现。
- 下一步最简单动作：加载现有连接器场景，同时薄适配对象/手/允许面与公开力学接口。
- 绝对禁止跑偏方向：续修 H102、对象专用坐标/阈值、磁吸/隐藏固定、在线读取对象/接触/PhysX 真值、写物体位姿、把退出 0 冒充物理成功。

## 当前权威字段

- 状态：`IMPLEMENTING`
- 当前里程碑：`ISAAC_CONNECTOR_SCENE_SMOKE_AND_MODEL_ADAPTER`
- V2 正式候选：空
- V2 研究动态门：`allowed=false`
...
- Isaac：6.0.1.0；最小场景 240 步、2.0 s；现有机器人资产 15 DOF、120 步运动通过；活动连接器待加载
