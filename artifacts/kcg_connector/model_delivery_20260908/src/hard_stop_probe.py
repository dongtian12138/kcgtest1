#!/usr/bin/env python3
"""ISOLATED_RIGID_CORE_STOP_COUPON_NOT_ASSEMBLY.

Only the source rigid core (with the seal partition excluded) and Socket interact. All other
...
p.add_argument("--known-force-n", type=float, default=0.)
p.add_argument("--velocity-iterations",type=int,choices=(0,1),default=1)
p.add_argument("--without-guide",action="store_true")
args = p.parse_args()
...
try:
    ...
    if not (.014 <= args.initial_depth_m < args.maximum_depth_m <= .01510001
    ...
    standalone.CreateSolverVelocityIterationCountAttr(args.velocity_iterations)
    disabled = []
    for prim in stage.Traverse():
        if prim.HasAPI(UsdPhysics.CollisionAPI) and str(prim.GetPath()) not in (seal_path, socket_path, *proxy_paths):
    ...
    guide = UsdPhysics.Joint.Define(stage, "/World/DeclaredRigidStopCouponAxialGuide")
    ...
    if args.without_guide:
        guide.CreateJointEnabledAttr(False)
        drive.GetMaxForceAttr().Set(0.);drive.GetStiffnessAttr().Set(0.);drive.GetDampingAttr().Set(0.)
    known_force=None
    if args.known_force_n:
        if not 0 < args.known_force_n <= 100.:
            raise ValueError('Declared isolated material load must be at most 100 N')
        drive.GetMaxForceAttr().Set(0.);drive.GetStiffnessAttr().Set(0.);drive.GetDampingAttr().Set(0.)
        known_force=PhysxSchema.PhysxForceAPI.Apply(body)
        known_force.CreateModeAttr('force');known_force.CreateWorldFrameEnabledAttr(True)
        known_force.CreateForceEnabledAttr(False);known_force.CreateForceAttr(Gf.Vec3f(0.))
        known_force.CreateTorqueAttr(Gf.Vec3f(0.))
    ...
    for phase, duration in (("initial_hold", .3), ("compression", args.stroke_duration_s),
        ...
        for i in range(count_steps):
            ...
            if phase == "compression":
                if known_force:known_force.GetForceAttr().Set(Gf.Vec3f(0.,0.,-args.known_force_n*fraction))
                else:drive.GetTargetPositionAttr().Set(stroke*fraction)
            elif phase == "withdrawal":
                if known_force:known_force.GetForceAttr().Set(Gf.Vec3f(0.,0.,-args.known_force_n*(1-fraction)))
                else:drive.GetTargetPositionAttr().Set(stroke*(1-fraction))
            if known_force:
                # Explicit diagnostic load only; use the immediate native
                # tensor call, not a USD force notice waiting for app updates.
                import warp as wp
                applied=wp.array(np.asarray([list(known_force.GetForceAttr().Get())],np.float32),dtype=wp.float32,device='cuda:0')
                loaded_indices=wp.array(np.asarray([0],np.int32),dtype=wp.int32,device='cuda:0')
                contact._physics_rigid_body_view.apply_forces(applied,loaded_indices,is_global=True)
            ...
            row = {"scope": "ISOLATED_RIGID_CORE_STOP_COUPON_NOT_ASSEMBLY", "step": step,
                   "declared_actuation": "TIME_ONLY_KNOWN_NATIVE_FORCE_AT_LINK" if known_force else "FINITE_NATIVE_POSITION_DRIVE",
                   "known_external_force_world_n":list(known_force.GetForceAttr().Get()) if known_force else None,
                   "native_mass_kg":float(host(contact.get_masses()).ravel()[0]),
    ...
    stream.close()
    world.pause()
    result = {"scope": "ISOLATED_RIGID_CORE_STOP_COUPON_NOT_ASSEMBLY", "source_scene": str(args.source),
