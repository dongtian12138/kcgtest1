...
`src/kcg_moveit1/` 提供机器人描述、ROS 2 与 MoveIt 支撑。当前动态工作集中在
`kcg_connector/grasp/` 与 `isaac/d38999_tabletop_pick_smoke.py`。
...
- `src/kcg_rl/`：早期 residual RL 研究代码。当前动态链未开放 RL 训练，不能把它
  当成现阶段完成路径。
...
活动 Isaac 运行器体积很大，但当前 B-V3 正在接入。为避免改变物理行为，本轮只清理
与它隔离的文档、工具和旧时间窗脚手架；运行器拆分应在 B 节点形成新的冻结动态基线后
单独进行。
