...
> 当前方法口径更新时间：2026-08-24T22:28:09Z
...
官方 GraspGenX、GPU 和 5 个 KCG 三指手描述器已经真实运行；按官方 object-PC 示例默认重放后，A/B 都只剩同一手型的 128 个提案，因此未被覆盖门接纳进入闭合、碰撞、任务、机械臂规划或 Isaac，50 mm 抬升和 2 s 保持尚未发生。

### 当前方法口径更新

- 当前生产参数除用户固定的 256 生成预算外，使用官方 object-PC 示例默认；A 为 `547→295→128`，B 为 `589→374→128`，最后都只含 `kcg_3f_preshape_02`。
- 两对象位置和方向仍覆盖四个掌侧，但“至少两个描述器”硬诊断失败，所以闭合、桌面、任务和 IK 均未执行。
...
- 两对象最终提案都只来自 `kcg_3f_preshape_02`。由于预登记的多描述器覆盖诊断失败，闭合预测、桌面与全手碰撞、任务受力、机械臂 IK 和 Isaac 均为 `NOT_RUN`。
...
3. 方法按每个手型最多保留 128 个方向分层姿态，再按六维多样性合并为最多 256 个候选；本次官方默认重放实际只有一个手型留下提案，所以在进入物理筛选前失败关闭。
...
两对象的手掌位置覆盖 4 个方位，roll、pitch 和径向距离均非恒定；但多描述器要求失败，因此完整六维覆盖诊断总体为失败。当前 128 个提案尚未经过闭合或碰撞检查，不能称为安全或不安全。
...
- 当前离线算法：A/B 均因最终提案只有一个描述器而在覆盖诊断失败；闭合、碰撞、任务和 IK 未运行。旧混合参数诊断中，A 在桌面路径门失败，B 在三指闭合预测门失败。
...
1. 本次固定输入、随机种子和官方默认阈值下，A/B 的最终提案都只保留一个描述器；尚不能由一次运行判定唯一原因。
2. KCG 模型条件配置箱明显超出本轮已审计的官方程序化及运行时描述器配置范围；它是否超出完整训练分布、以及是否导致描述器分布或旧诊断失败，都仍未验证。
3. 当前参数下闭合与碰撞未运行；旧混合参数诊断才观察到 A 的少量三指接触预测候选扫桌、B 没有三指允许接触预测。
...
“六维覆盖”不只看手掌方向，还要确认多个预构型真的留下了候选。对应证据链是：候选姿态和手型 → 关节闭合目标 → 登记网格的逐状态检查 → 指腹接触预测/连杆间隙 → 承重与抬升判据；当前默认重放在第一环就因单一手型失败关闭，所以后面的碰撞、受力、IK 和 Isaac 必须保持未运行。
