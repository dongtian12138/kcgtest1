...
> 离线几何与闭合预测结果冻结时间：2026-08-24T21:12:36Z
...
- 当前观察：六维覆盖范围变宽，但未转化为路径安全候选；KCG open 条件箱 `extents[1]` 为 `0.117526–0.305499 m`，高于 32 个官方程序化配置的已审计最大值 `0.067305 m` 和 26 个运行时描述器的 `0.060 m`。这是受支持的配置尺度差异，不是完整训练分布或唯一失败因果的证明。
...
1. KCG 模型条件配置箱明显超出本轮已审计的官方程序化及运行时描述器配置范围；它是否超出完整训练分布、以及是否导致本轮失败，都仍未验证。
...
5. 旧动态评价允许非 settle 阶段最多 `2 mm` 对象—桌面几何重叠通过，且没有登记来源。本路线没有运行该门；未来动态前必须先审查分阶段容差，否则不能宣称“无未授权穿透”。
