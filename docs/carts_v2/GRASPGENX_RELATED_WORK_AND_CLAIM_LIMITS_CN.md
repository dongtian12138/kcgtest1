...
如果同一预训练模型、描述器库、预算和物理门在对象 A/B 都产生路径安全且任务合格候选，并在冻结参数的 Isaac 运行中完成三指允许接触、离桌、50 mm、2 s，则支持本路线。当前结果与该假设不一致：A 没有三指闭合候选；B 有 2 个三指闭合候选，但两项在第一指闭合时已开始穿桌，第三指阶段最深约 4.46 mm。

当前数据支持的限制解释包括模型条件配置箱尺度外推，以及 released `sweep_volume_v2` 只读取 open/mid 工作箱、没有直接编码 `revolute_3f` 拓扑。另一个已证实的接口问题是官方 OBB 放置没有使用 KCG 非居中工作区的 x/y 偏移；统一补偿后 B 出现 2 个三指接触，证明它影响结果，但仍不足以产生安全路径。以上都不能证明“三指手物理上不能抓”，也不能被写成唯一失败根因；需要未来预先登记的独立方法验证。
