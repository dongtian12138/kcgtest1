...
当前数据支持的限制解释之一是模型条件配置箱的尺度外推，而不是“三指手物理上不能抓”。32 个程序化资产与 26 个运行时描述器的已审计配置范围都小于 KCG，但这不等于掌握了完整训练分布，也不能证明失败因果；该解释仍需未来用训练域匹配的描述器/模型或预先登记的替代全局提案器独立验证，不能在本轮通过继续调箱体、偏移或面标签来证实。
