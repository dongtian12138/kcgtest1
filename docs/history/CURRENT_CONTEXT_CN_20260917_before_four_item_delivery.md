...
核验时间：2026-09-17T12:29:39.940912+00:00。两次名义完整装配均已 VERIFIED。位姿变化案例 contact_last_gc128_pose_x1mm_yaw1deg 已实际退出码 2，归档封存 14720 帧。第 14719 步初始 parallel_contact_approach 触发 JOINT_SPEED_ABORT：f1j3 4.000000477 rad/s > 原 3 rad/s，未抓起、未进入装配。不是主机重启，不是舍入误停。session40317 和监控 cell198 已收尾，无主要物理进程。当前查接触/四杆约束最早原因，禁止继续按旧 ASSEMBLY_RUNNING 等待。

## 有效位置、代码与当前动作

- 活动树 /home/noob/WorkPlace/kcgtest1-improvements-20260916，分支codex/connector-assembly-improvements-20260916。HEAD ab129ee333b573dda98c9582d947b506f5e1ea74已提交推送，包含两轮成功证据和后续等价审计/后处理改动。当前仅本上下文跟进可变状态。原树/home/noob/WorkPlace/kcgtest1用户原脏资产保留，仅镜像本上下文；不变基线树保持原样。
...
- 当前未提交资料含docs/ASSEMBLY_REPEATABILITY_20260917_CN.md、第二轮与控制诊断/性能小证据、first_verified_fresh_checkout_check.json、second_recording_optimization.json、上述2源+后评入口+manifest。这些修改已随ab129ee提交推送。pose-variation新预检已启动，随后入口自动启动完整动作；现在不得再改生产源。
...
1. ab129ee已保存提交推送，两轮原执行版本及manifest未回写。位姿变化已结束且提前停止，正在核对初始接触失效原因。
2. 已执行入口：python3 scripts/run_current_hand_assembly.py --run --pose-variation --output-root artifacts/full_validation/contact_last_gc128_pose_x1mm_yaw1deg，session40317 已退出，目录已存在不可复用。启动manifest SHA 1ce843904f0767c2ae6cc20955ff6d72a0380f12da69a23d5896435cb8db6887。配置已准备，初始平移[.001,0,0]m、yawπ/180，摩擦仍.45，所有变化在物理启动前。先新预检，固定21600s墙钟/1200s收尾，不运行中扩限；不要对bounded_experiment.py调用--help。
