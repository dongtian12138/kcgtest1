...
核验时间：2026-09-18T18:28:31.933545+00:00。四相机full_chain_04已通过完整装配与独立3秒释放验收。唯一物理回合为+1mm/+1°初抓扰动前缀。
...
- 实现树`/home/noob/WorkPlace/kcgtest1-performance-20260917`，分支`codex/four-camera-single-key-20260918`，HEAD 84bef70。原`/home/noob/WorkPlace/kcgtest1`只同步本上下文，其它脏资产不碰。未推送。
...
唯一主物理回合`artifacts/four_camera_20260918/variation_plus_grasp_01`：2026-09-18T18:17:30.011584Z启动，bounded PID3636656，shell session69396，source84bef70。900s/120s固定预算，仅初始视觉抓取、抬升、2s保持，不看键/搬运/旋拧；验证旧+1mm/+1°最早失效点。配置visual_body_centered_preload_variation.yaml，scenario initial_x_plus_1mm_yaw_plus_1deg_mu0p45，预检variation_plus_preflight_01已通过（207.366s，2026-09-18T18:14:12.876265Z结束，exit0，四门全true）。恢复先查process.json和实际进程。源码冻结，不SIGINT、不在线延长；受控停止写run/STOP_REQUEST。

full_chain_04已于2026-09-18T17:59:51.662992Z结束，9073.704s（约2h31m14s）、273285物理步、source84bef70924b7244f47493a75c38a17fade6c3b58；进程exit2仍是旧通用评价标签。**whole_assembly_review.complete_visual_assembly_verified=true / VERIFIED，独立three_second_release_review.accepted=true**。初抓NAIL、原两甲一腹Nut区域、源键、外观、四相机、引擎、身份、真值隔离均通过。初始预检full_chain_preflight_02已通过193.8994s、exit0，物理绑定未变。

- 实际插头两段搬运0.3970812+0.0603910=0.4574722m；手基坐标点0.4431966m；搬运峰值arm.120988rad/s<原.15。
- 4条功能感知相机路径、G2键1次、掌心1109帧延迟消费、固定手眼外参审查通过。本体卸载前retire，不随Nu手回转传播Body坐标关系。
- 开始接触时单键轴向误差.0421348°，抓持生命周期最大.2726773°，中心最大21.627µm、轴.009570°；5DOF仍不观测额外轴向转动。
- 最终3秒[270405,273285)共2880帧，手部原始冲量逐帧严格零、源止挡逐帧正载、同128簧套逐帧正载、Body/Nut醒着；实际深度14.603955mm恒定。全回合最大备用轴坐标.522465mm<原.5999mm。
- 源键顶点对槽面最大残差1.175528µm<原2µm。源带径向1024/1024样本遮挡，已查看同回合旋拧末/释放中/最终原始视频帧，无露出红带；报告保留图像范围边界。
- 在线到位True：两独立视觉深度14.603779/14.603582mm，实际张手保持3s、加载扭矩2.003314Nm；力/换抓停止本身未算完成。第四次在累计356.248994°触发原弹性裕量换抓，第五次仅再转约1.708548°后触发到位候选并最终释放；原360°上界未放宽。
- Nut首次规划8.628619s由8284物理步保持后消费，首次回转2.246249s由2157步保持后消费；Nu相位规划/验证亦有真实等待。te_visual_seating_axis可选分段分支当前配置没有启用，该分支只做了软件检查，不宣称动态覆盖。
- 完整报告`reproducibility/four_camera_20260918/full_assembly_result_CN.md`；精简证据`reproducibility/four_camera_20260918/evidence/full_chain_04/`已复制并以report_integrity绑定。原始数据仍在artifacts原位，未删除；raw msgpack.gz16342211301bytes，录像44762666bytes。
- 与旧15909.99s进程记录相比约1.7534倍，**未达到原5倍整轮提速**。独立后验检查另计。不把7.6倍路程比写成整轮速度比。当前只验证一个完整名义场景。
...
接下来：跟进+1mm/+1°初抓前缀，结束后按真实NAIL/抬升/保持复核；再对称执行-1mm/-1°初抓前缀及对应预检。仅一个主回合、21600s/240s收尾，不在线延长。配置visual_body_centered_preload.yaml/assembly_fixed_cameras.yaml及原arm seed[0,0,0,-pi/2,0,pi/2,2.5]、friction_lower_0p45、parallel_contact_latch、msgpack/native-report/5fps保持。之后执行已定义的小扰动；当前扰动0次，完整整机0次成功。
...
1. full_chain_04完整验收通过；当前只剩两组已定义初抓扰动与结果整理。恢复先查最新process.json及进程，不凭历史“运行中”等待。运行期间不改加载源码；失败则诊断最早原因，不盲扫或放宽门。
