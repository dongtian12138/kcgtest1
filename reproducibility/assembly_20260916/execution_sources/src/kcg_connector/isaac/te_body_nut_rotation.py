...
def run_body_nut_rotation(repository, runtime, stepper, dynamic, grip,
    ...
    if settings.get('loaded_coordination',{}).get('enabled',False) and settings.get('in_turn_feedback',{}).get('enabled',False):
        raise ValueError('coordinated rotation cannot be combined with the legacy segmented reference controller')
...
def _run_body_nut_rotation_interval(repository, runtime, stepper, dynamic, grip,
    ...
    if settings.get('coaxial_interface_recipe'):
        ...
        return run_coaxial_nut_interval(repository,runtime,stepper,dynamic,grip,
            world_from_socket,settings,output)
    import omni.replicator.core as rep
    import omni.usd
    from scipy.spatial.transform import Rotation
    from te_nut_motion import (ScalarMotion,limit_joint_velocity_without_changing_direction,
        classify_observed_progress,LoadedTurnProgress,coordinated_turn_scale,requires_axial_unload)
    ...
    axial_correction=0.
    coordination=settings.get('loaded_coordination',{})
    coordinated=bool(coordination.get('enabled',False))
    successful_stroke=bool(settings.get('successful_stroke_motion',{}).get('enabled',False))
    successful_tilt=np.eye(3)
    if successful_stroke and (coordinated or axial_lead_enabled or not tracking_enabled
            or settings.get('arm_kinematic_reference')!='measured_pose' or visual_follow_enabled):
        raise ValueError('successful-stroke transfer requires measured-pose feedback and its own force/tilt reference')
    if coordinated and not (tracking_enabled and axial_lead_enabled):
        raise ValueError('loaded coordination requires measured tracking and axial lead following')
    turn_progress=LoadedTurnProgress(angle,maximum_speed,maximum_acceleration) if coordinated else None
    progress_candidate=None;coord_previous_error=None;coord_error_growth=0.
    coord_scale=1.;coord_prediction=0.;coord_finished_s=None
    coord_extra=float(coordination.get('maximum_extra_time_s',.5)) if coordinated else 0.
    coord_paused_s=0.;coord_resume_count=0;coord_was_paused=False
    load_bias_transferred=False
    last_applied_load_compensation=np.zeros(7)
    planned_pause=coordination.get('planned_pause',{}) if coordinated else {}
    pause_done=False;pause_zero_start=None
    if planned_pause:
        pause_angle=float(planned_pause['after_command_deg']);pause_hold=float(planned_pause['hold_s'])
        if not 0<pause_angle<abs(np.degrees(angle)) or not 0<pause_hold<=.5:
            raise ValueError('one bounded interior pause is required')
        coord_extra+=pause_hold+maximum_speed/maximum_acceleration
    if coordinated and settings.get('continue_established_axial_force_reference',False):
    ...
    try:
        ...
        planar_enabled = bool(planar.get("enabled", False)) and (coordinated or not initial_fit_accepted)
        ...
        save()
        count = round((alignment_duration + settle_s + duration + hold_s + rate_extra_time + coord_extra) / dt)
        ...
        while index < count:
            ...
            if root_observer is not None and (regulate_finger_effort or regulate_preparation_effort):
                ...
                if motor_force_control.get('enabled',False):
                    root_moment_filtered=root_observer.filter_moments(root_moments,stepper.step_index,dt,
                        float(motor_force_control['signal_filter_time_s']))
            ...
            if visual_feedback is not None and not feedback_verified and elapsed >= preparation_end:
                ...
                if pose_accepted:
                    ...
                    count = round((preparation_end + duration + hold_s + rate_extra_time + coord_extra) / dt)
            ...
            if rate_guard_enabled and not coordinated and u >= 1. and rate_turn_finished_s is None:
            ...
            engagement_progress_deg = engagement_start_deg + abs(float(np.degrees(
                turn_progress.position if coordinated else angle*fraction)))
            if engagement_enabled:
            ...
            if coordinated and elapsed>=preparation_end:
                e=desired_pivot[:3,3]-current[:3,3]
                lateral=float(np.linalg.norm(e-axis*(axis@e)))
                cv=wrench[2]-force_reference
                cv=cv*float(settings['axial_admittance_m_per_n_s'])-float(axial_lead['restoring_rate_s_inv'])*axial_correction
                bound=float(axial_lead['maximum_correction_m'])
                effective_cv=(float(np.clip(axial_correction+cv*dt,-bound,bound))-axial_correction)/dt
                unloading=requires_axial_unload(effective_cv,np.copysign(turn_progress.speed,angle),
                    float(axial_lead['lead_m']),float(coordination.get('outward_unload_threshold_m_s',.00003)))
                coord_scale,coord_error_growth,coord_prediction=coordinated_turn_scale(
                    lateral,coord_previous_error,coord_error_growth,turn_progress.speed,maximum_acceleration,
                    float(coordination['begin_slowing_error_m']),float(measured_tracking['maximum_lateral_error_m']),dt,unloading)
                coord_previous_error=lateral
                if planned_pause and not pause_done and abs(np.degrees(turn_progress.position))>=pause_angle:
                    if turn_progress.speed<1e-12:
                        if pause_zero_start is None:pause_zero_start=elapsed
                        pause_done=elapsed-pause_zero_start>=pause_hold
                    if not pause_done:coord_scale=0.
                progress_candidate=turn_progress.candidate(dt,min(coord_scale,turn_rate_scale))
                fraction=progress_candidate[0]/angle if abs(angle)>1e-15 else 1.
                rotation_velocity=progress_candidate[2]
            ...
            if successful_stroke and elapsed>=preparation_end:
                from te_nut_motion import successful_stroke_tilt
                successful_tilt,tilt_velocity=successful_stroke_tilt(
                    successful_tilt,socket[:3,:3],wrench[3:5],dt)
                desired_pivot[:3,:3]=successful_tilt@desired_pivot[:3,:3]
                follow_angular_velocity=tilt_velocity+successful_tilt@follow_angular_velocity
                follow_rotation=successful_tilt@follow_rotation
                record['successful_stroke_motion']={
                    'source_run':settings['successful_stroke_motion']['source_run'],
                    'position_feedback_gain_s_inv':20.,'orientation_feedback_gain_s_inv':3.,
                    'tilt_offset_deg':Rotation.from_matrix(successful_tilt).as_rotvec(degrees=True).tolist(),
                    'finger_motor_interface':'CURRENT_FOURBAR_SELF_LOCK_FINITE_INVERSE',
                    'old_finger_targets_or_effort_caps_copied':False}
            ...
            control_sample = _json_ready({"step": int(stepper.step_index), "elapsed_s": elapsed,
                ...
                "coordinated_turn_scale":coord_scale if coordinated else None,
                "coordinated_predicted_lateral_error_m":coord_prediction if coordinated else None,
                "coordinated_paused_time_s":coord_paused_s if coordinated else None,
                "coordinated_resume_count":coord_resume_count if coordinated else None,
                "planned_pause_completed":pause_done if planned_pause else None,
            ...
            if successful_stroke:
                control_sample['successful_stroke_tilt_deg']=Rotation.from_matrix(successful_tilt).as_rotvec(degrees=True).tolist()
            ...
            if coordinated:
                jacobian=np.asarray(inputs.robot_model.geometric_jacobian('handbase_link',tuple(q),
                    point_local_m=hand_from_pivot[:3,3]))[:,:7]
                point_jacobian=np.vstack((jacobian[:3],.05*jacobian[3:]))
            ...
            if successful_stroke:
                from te_nut_motion import successful_stroke_joint_velocity
                joint_velocity,velocity_residual=successful_stroke_joint_velocity(
                    point_jacobian,twist,error,arm,bounds,float(settings['maximum_arm_speed_rad_s']))
                record['successful_stroke_velocity_residual']=velocity_residual.tolist()
            if coordinated:
                # Bound the combined task command, including position feedback.
                task_velocity=twist+3.*error
                linear_cap=float(coordination['maximum_total_linear_speed_m_s'])
                angular_cap=float(coordination['maximum_total_angular_speed_rad_s'])
                scale=min(1.,linear_cap/max(np.linalg.norm(task_velocity[:3]),1e-15),
                    angular_cap/max(np.linalg.norm(task_velocity[3:])/.05,1e-15))
                joint_velocity*=scale
                arm_before_increment=arm.copy()
            if tracking_enabled:
            ...
            if balance_target is not None:
                ...
                if motor_force_control.get('enabled',False):
                    ...
                    hand_target,motor_rows=finger_force_motor_targets(world.hand_mechanism,q[8:],hand_target,
                        desired_effort,root_moment_filtered,dt,float(settings['finger_position_stiffness_reference_nm_rad']),
                        effort_tau,float(dynamic['finger_maximum_speed_rad_s']),float(motor_force_control['moment_deadband_nm']),
                        lower_hand,upper_hand)
            ...
            if coordinated and elapsed>=preparation_end:
                phase='turn' if abs(turn_progress.position-angle)>1e-10 else 'hold'
            ...
            if load_compensation.get('enabled',False):
                ...
                if not 0<gain<=1. or not 0<limit<=20.:
                ...
                if not load_bias_transferred:
                    # Transfer the load already held by the position target into
                    # explicit feedforward without changing the native drive target.
                    bias=compensation/float(stepper.settings['arm_stiffness'])
                    arm-=bias
                    if coordinated:arm_before_increment-=bias
                    if np.any(arm<bounds[:,0]) or np.any(arm>bounds[:,1]):
                        raise RuntimeError('load reference transfer exceeds the original arm limits')
                    hit=check(arm,q[7:],nut_contact=True)
                    if hit is not None:raise RuntimeError(f'load reference transfer geometry: {hit}')
                    record['load_reference_transfer']={'position_bias_removed_rad':bias.tolist(),
                        'requested_native_target_change_from_activation_rad':0.,'applied':False,
                        'object_pose_written':False}
            if coordinated:
                # The same native drive equation and limits used by advance.
                # Reject an infeasible increment before committing any trajectory state.
                payload=(np.zeros(7) if stepper.payload_model is None else control.payload_compensation_joint_torque(
                    inputs.robot_model,np.r_[arm,hand_target],stepper.payload_model,stepper.payload_compensation_fraction))
                _,requested=control.gravity_biased_arm_target(stepper.robot,stepper.arm_indices,arm,
                    stepper.arm_lower_limits,stepper.arm_upper_limits,stepper.settings,
                    arm_damping_nm_s_rad=stepper.effective_arm_damping_nm_s_rad,
                    payload_feedforward_nm=payload,velocity_reference_rad_s=joint_velocity,
                    load_feedforward_nm=advance_options.get('arm_load_compensation_nm'))
                if requested['saturated']:
                    arm=arm_before_increment
                    raise RuntimeError('recoverable nut turn stop: INFEASIBLE_REFERENCE_INCREMENT')
                record['loaded_coordination']={'requested_scale':coord_scale,'predicted_lateral_error_m':coord_prediction,
                    'phase_angle_deg':float(np.degrees(turn_progress.position)),
                    'phase_speed_deg_s':float(np.degrees(turn_progress.speed)),
                    'paused_time_s':coord_paused_s,'resume_count':coord_resume_count,
                    'planned_pause_completed':pause_done if planned_pause else None,
                    'planned_pause_zero_reference_start_s':pause_zero_start,
                    'drive_request_max_nm':float(np.max(np.abs(requested['raw_total_effort_nm'])))}
            ...
            if int(stepper.step_index)>before_step:
                last_applied_load_compensation=np.asarray(advance_options.get('arm_load_compensation_nm',np.zeros(7))).copy()
                if load_compensation.get('enabled',False):
                    load_bias_transferred=True
                    record['load_reference_transfer']['applied']=True
                ...
                if coordinated and progress_candidate is not None:
                    turn_progress.commit(progress_candidate)
                    paused=turn_progress.speed<.01*maximum_speed and coord_scale<.05
                    coord_paused_s+=dt*paused
                    if coord_was_paused and turn_progress.speed>.1*maximum_speed:coord_resume_count+=1;coord_was_paused=False
                    if paused:coord_was_paused=True
                    if abs(turn_progress.position-angle)<1e-10 and coord_finished_s is None:coord_finished_s=elapsed
            elif coordinated:
                arm=arm_before_increment
            ...
            index += 1
            if coordinated and coord_finished_s is not None and elapsed-coord_finished_s>=hold_s:break
        if stepper.abort_reason is not None:
            raise RuntimeError(stepper.abort_reason)
        if coordinated and not interval_paused and (coord_finished_s is None or elapsed-coord_finished_s<hold_s):
            raise RuntimeError('recoverable nut turn stop: COORDINATED_PROGRESS_DID_NOT_RECOVER')
        ...
        world.pause()
        if load_bias_transferred:
            record['load_compensated_hold_arm_target_rad']=(
                np.asarray(ft.samples[-1]['active_targets_rad'][:7])
                +last_applied_load_compensation/float(stepper.settings['arm_stiffness'])).tolist()
            record['last_applied_load_compensation_nm']=last_applied_load_compensation.tolist()
