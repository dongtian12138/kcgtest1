...
class HandMechanismRuntime:
    ...
    def step(self, *args, **kwargs):
        ...
        if properties!=self._authored_drive_properties:
            ...
            native_caps=host(self.robot.get_dof_max_efforts(indices=0,dof_indices=self.indices)).reshape(-1)
            explicit_efforts=host(self.robot.get_dof_efforts(indices=0,dof_indices=self.indices)).reshape(-1)
            if not np.allclose(native_caps,[r['max_effort'] for r in laws],rtol=0.,atol=1e-6):
                raise RuntimeError('Native hand drive caps do not match the finite requested limits')
            if not np.isfinite(explicit_efforts).all() or np.max(np.abs(explicit_efforts))>1e-9:
                raise RuntimeError('Unaccounted explicit hand joint actuation effort')
            self.native_drive_limit_readback=native_caps.tolist()
        ...
        if not np.isfinite(np.r_[after,velocity,effort]).all():raise RuntimeError('Nonfinite native hand state or joint reaction')
        ...
        for i,name in enumerate(ACTIVE_HAND):
            reaction=float(effort[self.indices[i]]+slopes[name]*effort[self.followers[i]])
            # This API returns the incoming joint wrench projected onto each
            # motion axis. Its source/follower combination is not an isolated
            # native-drive force sensor, especially during constraint impulses.
            row=self.drives[name].complete(float(after[self.indices[i]]),float(velocity[self.indices[i]]),
                native_capped_drive=True)
            row.update(projected_joint_reaction_combination_nm=reaction,
                projected_source_joint_reaction_nm=float(effort[self.indices[i]]),
                projected_follower_joint_reaction_nm=float(effort[self.followers[i]]),
                applied_constraint_slope=float(slopes[name]),
                native_drive_maximum_effort_readback_nm=self.native_drive_limit_readback[i],
                joint_reaction_not_used_as_drive_limit_measurement=True)
