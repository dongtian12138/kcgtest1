...
def child(mode):
 ...
 try:
  with gzip.open(SOURCE,'rb') as stream:
   for source_row in itertools.islice(ijson.items(stream,'item',use_float=True),249000,249000+COUNT):
...
if __name__=='__main__':
 if len(sys.argv)>1:child(sys.argv[1])
  ...
  result={'scope':'BOUNDED_OFFLINE_32768_LATE_EPISODE_REAL_SENSOR_RECORDS','original':a,'candidate':b,'collection_cost_reduction_fraction':1-b['steady_collection_median_s']/a['steady_collection_median_s'],'not_a_whole_episode_speed_estimate':True,'not_adopted_into_live_physics':True}
