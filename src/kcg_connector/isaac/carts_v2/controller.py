...
class JointSignalStepper:
    ...
    def _advance_step(
        ...
        *, pre_step_hook=None, arm_velocity_target=None, arm_load_compensation_nm=None,
        ...
        stop_path=getattr(self,'diagnostic_stop_request_path',None)
        if stop_path and os.path.exists(stop_path):
            self.abort_reason='DIAGNOSTIC_STOP_REQUESTED'
            return self.latest
        step_limit=getattr(self,'diagnostic_step_limit',None)
        if step_limit is not None and self.step_index>=step_limit:
            self.abort_reason='DIAGNOSTIC_STEP_BUDGET_REACHED'
            return self.latest
...
def _tare_and_close(stepper, motion_plan, settings, pregrasp, first_finger_only=False):
    ...
    for _ in stepper.active_steps(tare_steps):
        ...
        if stepper.abort_reason is None:
            ...
            tare_encoder_rows.append(latest[0].copy())
    tare = np.mean(np.stack(tare_rows), axis=0) if tare_rows else np.zeros(4)
    if (tare_rows and getattr(stepper,'hand_mechanism',None) is not None
            and getattr(stepper,'robot_model',None) is not None):
        from te_three_finger_wrench_observer import FingerRootMomentObserver
        observer=FingerRootMomentObserver(Path(__file__).resolve().parents[4],stepper.robot_model)
        observer.calibrate_free_space(tare_encoder_rows,np.asarray(tare_rows)[:,1:])
        stepper.finger_moment_observer=observer
    ...
    for _ in stepper.active_steps(closure_steps):
        ...
        if getattr(stepper,'finger_moment_observer',None) is not None:
            effort_delta[1:]=stepper.finger_moment_observer.observe(stepper.latest[0],stepper.latest[2][8:])
    ...
    for _ in stepper.active_steps(hold_steps):
        ...
        if getattr(stepper,'finger_moment_observer',None) is not None:
            effort[1:]=stepper.finger_moment_observer.observe(stepper.latest[0],stepper.latest[2][8:])
        hand_target = contact.step(
            measured_hand,
            effort,
            maximum_increment,
...
def _run_preload_lift_hold(stepper, motion_plan, settings, pregrasp, contact, tare, final_hand):
    ...
    def regulated_hand_target(
        ...
        if observer is not None:
            ...
            measured=observer.control_moments(stepper.latest[0],stepper.latest[2][8:],stepper.step_index,dt)
    ...
    observer=getattr(stepper,'finger_moment_observer',None)
    initial_preload_effort=(closing_direction*observer.observe(stepper.latest[0],stepper.latest[2][8:])
        if observer is not None else np.zeros(3))
    for index in stepper.active_steps(preload_steps):
        ...
        hand_target = regulated_hand_target(
            previous_target, initial_preload_effort+blend*(predicted_task_effort-initial_preload_effort)
        )
    ...
    for _ in stepper.active_steps(effort_check_steps):
        ...
        if measured_effort_requires_abort(settings, measured_effort):
            return "HAND_MEASURED_EFFORT_ABORT"
        if observer is not None:measured_effort=observer.observe(latest[0],latest[2][8:])
        resistive_effort = closing_direction * measured_effort
