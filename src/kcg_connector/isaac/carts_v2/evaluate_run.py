...
class TruthAuditRecorder:
    ...
    def _tensor_contact_rows(self) -> list[dict[str, object]]:
        ...
        for sensor_index, sensor_path in enumerate(
            ...
            other_paths=[self._tensor_actor_paths[i] for i in id_values]
            ...
            # Convert each contiguous native slice once. All points and their
            # order are retained; only redundant per-point NumPy views go away.
            points_values=points_array[start:end].tolist()
            normal_values=normals_array[start:end].tolist()
            force_values=forces_array[start:end].tolist()
            impulse_values=(normals_array[start:end]*forces_array[start:end,None]).tolist()
            separation_values=separations_array[start:end].tolist()
            for other_path,actor_id,point,normal,impulse,vector,separation in zip(
                    other_paths,id_values,points_values,normal_values,force_values,impulse_values,separation_values):
                records=grouped.get(other_path)
                if records is None:
                    records=[];grouped[other_path]=records
                records.append({
    ...
    def _contact_counts(self) -> dict[str, object]:
        ...
        result: dict[str, object] = {
            ...
            "contact_observation_backend": "native_full_contact_report" if native_only else "tensor_raw_contact",
            "tensor_contacts_available": not native_only,
            "friction_data_available": not native_only,
            "uncollected_contact_channels": ['tensor_raw_contact','tensor_friction'] if native_only else [],
            "all_provided_native_report_points_retained": True,
...
def _contact_surface_slip_metrics(
    ...
    friction_channel_present = all(
        "friction_headers" in row["contacts"] and row['contacts'].get('friction_data_available',True)
        for row in samples
    )
    per_finger = []
