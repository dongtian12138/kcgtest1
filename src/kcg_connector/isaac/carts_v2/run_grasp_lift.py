...
def _arguments(repository: Path) -> argparse.Namespace:
    ...
    parser.add_argument('--truth-archive-codec',choices=('jsonl','msgpack'),default='jsonl',
        help='lossless codec for the bounded shared-hand observation archive')
    parser.add_argument('--contact-audit-mode',choices=('full','native-report'),default='full',
        help='retain native contact points and optionally omit redundant tensor/friction diagnostics')
    parser.add_argument(
...
def _create_runtime(
    ...
    auditor = TruthAuditRecorder(
        ...
        packed_native_contacts=(assembly_document.get('recording',{}).get('packed_native_contacts',False)
                                if arguments.body_assembly_collision_config else False),
        native_contact_float32=(assembly_document.get('recording',{}).get('native_contact_float32',False)
                                if arguments.body_assembly_collision_config else False),
    )
    truth_stream = None
...
def _finish_run(repository, inputs, runtime, trace, outcome):
    ...
    if runtime.get("truth_stream") is not None:
        ...
        trace["durable_truth_samples"] = {"path": runtime["truth_stream"].name,
            "format": ('MSGPACK_GZIP' if getattr(runtime['truth_stream'],'codec',None)=='msgpack'
                       else "JSONL_GZIP" if str(runtime["truth_stream"].name).endswith(".gz") else "JSONL"),
            "returned_to_controller": False}
    trace["controller_outcome"] = outcome
