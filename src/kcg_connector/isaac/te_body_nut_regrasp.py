...
def run_body_nut_regrasp(repository, runtime, stepper, dynamic, observation,
    ...
    capture_scale=float(config['nut_regrasp'].get('initial_capture_preload_scale',1.))
    if not 0<capture_scale<=1.:raise ValueError('Initial capture may only reduce the established grip reference')
    if root_preload and capture_scale<1. and float(runtime.get('coaxial_nut_commanded_degrees',0.))<80.:
        import copy
        root_preload=copy.deepcopy(root_preload)
        root_preload['targets_nm']=(capture_scale*np.asarray(root_preload['targets_nm'])).tolist()
        if root_preload.get('task_refinement'):
            root_preload['task_refinement']['targets_nm']=(capture_scale*np.asarray(root_preload['task_refinement']['targets_nm'])).tolist()
        root_preload['initial_capture_reference_scale']=capture_scale
    ...
    retained_reindex_body_rotation=None
    yaw_bias_config=config['nut_regrasp'].get('grasp_yaw_bias',{})
    yaw_bias=float(yaw_bias_config.get('degrees',0.))
    yaw_bias_start=float(yaw_bias_config.get('minimum_cumulative_command_deg',0.))
    previous_yaw_bias=float(runtime.get('nut_grasp_yaw_bias_deg',0.))
    if (not np.isfinite([yaw_bias,yaw_bias_start,previous_yaw_bias]).all()
            or max(abs(yaw_bias),abs(previous_yaw_bias))>5.
            or not 0<=yaw_bias_start<=360.):
        raise ValueError('The declared grasp yaw bias is limited to five degrees')
    if yaw_bias and not config['nut_regrasp'].get('keyed_socket_grasp_frame',False):
        raise ValueError('A grasp yaw bias requires the current visual keyed frame')
    desired_yaw_bias=(yaw_bias if float(runtime.get('coaxial_nut_commanded_degrees',0.))+1e-6>=yaw_bias_start else 0.)
    visual_phase_settings=config['nut_regrasp'].get('visual_nut_phase',{})
    visual_phase_start=float(visual_phase_settings.get('minimum_cumulative_command_deg',180.))
    visual_phase_active=bool(visual_phase_settings.get('enabled') and
        float(runtime.get('coaxial_nut_commanded_degrees',0.))+1e-6>=visual_phase_start)
    if visual_phase_settings.get('enabled') and not 80.<=visual_phase_start<=360.:
        raise ValueError('Visual Nu phase starts only after the captured-interface stage')
    if visual_phase_active and (abs(yaw_bias)>1e-9 or abs(previous_yaw_bias)>1e-9):
        raise ValueError('Visual phase alignment does not combine with a fixed grasp-yaw bias')
    if visual_phase_active and not config['nut_regrasp'].get('keyed_socket_grasp_frame',False):
        raise ValueError('Visual Nu phase requires the current keyed Socket reference')

    def target_from_palm(measurement):
        nonlocal retained_reindex_body_rotation
        ...
        if config['nut_regrasp'].get('keyed_socket_grasp_frame',False):
            # Source Body keys fix the assembly's nominal azimuth in the
            # currently observed Socket frame. The old cylindrical PAD
            # adapter kept arbitrary hand yaw, which does not preserve the
            # tested NAIL/flute grasp orientation. This is a declared keyed
            # assembly reference, not a direct measurement of Nut yaw.
            keyed=np.eye(4);keyed[:3,3]=body[:3,3]
            ...
            if keep_reindex_yaw:
                if retained_reindex_body_rotation is None:
                    ...
                    retained=current_hand[:3,:3]@canonical[:3,:3].T
                    axis=retained[:,2];wanted=-np.asarray(world_from_socket)[:3,2]
                    cross=np.cross(axis,wanted);sine=np.linalg.norm(cross)
                    cosine=float(np.clip(axis@wanted,-1.,1.))
                    if cosine<.5:raise ValueError('Returned hand axis is inconsistent with the visual Socket')
                    if sine>1e-12:
                        retained=Rotation.from_rotvec(cross*np.arctan2(sine,cosine)/sine).as_matrix()@retained
                    retained_reindex_body_rotation=retained
                keyed[:3,:3]=retained_reindex_body_rotation
            if float(runtime.get('coaxial_nut_commanded_degrees',0.))>=80.:
                axial=float(config['nut_regrasp'].get('captured_nut_offset_m',0.))
                if not 0<=axial<=.0005000001:
                    raise ValueError('Captured Nut reference must remain inside the source working thrust travel')
                keyed[:3,3]+=keyed[:3,2]*axial
                record['captured_nut_axial_reference_m']=axial
            ...
            # its collision envelope unchanged. A retained return attitude
            # already contains the previous declared bias; do not add it twice.
            increment=desired_yaw_bias-previous_yaw_bias if keep_reindex_yaw else desired_yaw_bias
            if abs(increment)>5.:raise ValueError('One grasp yaw change may not exceed five degrees')
            grasp_frame=keyed.copy()
            if increment:
                grasp_frame[:3,:3]=keyed[:3,:3]@Rotation.from_euler('z',increment,degrees=True).as_matrix()
            record['grasp_yaw_bias']={'desired_total_bias_deg':desired_yaw_bias,
                'previous_total_bias_deg':previous_yaw_bias,'applied_increment_deg':increment,
                'axis':'KEYED_BODY_PLUS_Z_OPPOSITE_VISUAL_SOCKET_PLUS_Z',
                'source':'DECLARED_FIXED_GRASP_POSE_PARAMETER_NOT_MEASURED_NUT_YAW',
                'body_observation_and_collision_envelope_rotated':False,
                'physical_object_pose_written':False}
            return body,grasp_frame@canonical
        _, hand = hand_pose()
    ...
    try:
        ...
        if root_preload:
            ...
            force_ceiling=110. if config.get('nut_rotation',{}).get('coaxial_interface_recipe') else 20.
            if not np.isfinite(reference_force) or not 0<reference_force<=force_ceiling:
        ...
        if visual_phase_active:
            ...
            record['visual_nut_phase_start_position_check']={
                'error_m':initial_phase_position_error,'original_tolerance_m':initial_phase_position_tolerance,
                'passed':initial_phase_position_error<=initial_phase_position_tolerance}
            if initial_phase_position_error>initial_phase_position_tolerance:
                raise RuntimeError('the fresh palm pose leaves insufficient clearance for the planned nut grip')
            from te_nut_phase_vision import estimate_for_grasp, grasp_target_from_phase, grasp_phase_error_deg
            phase_settings={**visual_phase_settings,
                'captured_nut_offset_m':float(config['nut_regrasp'].get('captured_nut_offset_m',0.))}
            phase_cache=runtime.setdefault('nut_phase_image_template_cache',{})
            commanded=float(runtime.get('coaxial_nut_commanded_degrees',0.))
            phase=estimate_for_grasp(repository,fresh,world_from_socket,hand,commanded,phase_settings,phase_cache)
            record['visual_nut_phase_before_alignment']=phase
            corrected_target,phase_increment=grasp_target_from_phase(new_target,canonical,phase)
            record['visual_nut_phase_alignment']={'applied_yaw_increment_deg':phase_increment,
                'maximum_yaw_increment_deg':5.,'target_world_from_hand':corrected_target.tolist(),
                'source':'CURRENT_RGBD_NU_EXTERIOR_AND_FIXED_REFERENCE_GRASP',
                'object_pose_written':False,'body_observation_or_envelope_rotated':False}
            record['stage']='ALIGNING_OPEN_HAND_TO_OBSERVED_NUT_PHASE'
            save()
            active,_=hand_pose()
            phase_states,phase_ik=_plan_key_probe_descent(inputs,active,corrected_target,dt,transfer_speed)
            phase_offset=np.asarray(ft.samples[-1]['active_targets_rad'][:7])-active[:7]
            phase_states+=phase_offset
            phase_peak=float(np.max(np.abs(np.diff(phase_states,axis=0)))/dt) if len(phase_states)>1 else 0.
            if (phase_peak>transfer_arm_speed or np.any(phase_states<soft[:,0]) or np.any(phase_states>soft[:,1])):
                raise RuntimeError('Visual phase alignment exceeds the existing joint bounds')
            for index,arm in enumerate(phase_states):
                hit=check(arm,open_goal)
                if hit is not None:
                    record['planned_phase_alignment_collision']={'sample':index,**hit}
                    raise RuntimeError('Visual phase alignment collides with the original avoidance geometry')
            record['motions'].append({'kind':'VISUAL_NUT_PHASE_ALIGNMENT','ik':phase_ik,
            ...
            for arm in phase_states:
                advance('key_probe_nut_transfer',arm,open_goal)
            held_arm=phase_states[-1].copy()
            for _ in range(round(.5/dt)):
                advance('key_probe_nut_transfer_hold',held_arm,open_goal)
            world.pause()
            _,hand=hand_pose()
            record['pre_phase_alignment_observation']=fresh
            fresh=observe_released_plug_from_rgbd(repository,stage,world,rep,hand,output/'phase_aligned_palm')
            record['phase_aligned_palm_observation']=fresh
            record['preclose_palm_observation']=fresh
            ...
            record['visual_nut_phase_alignment'].update(
                motion_completed=True,actual_target_tracking_yaw_error_deg=yaw_error,
                verification_yaw_tolerance_deg=.5)
            if abs(yaw_error)>.5:
                raise RuntimeError('Fresh image still shows a misaligned Nu grasp after the bounded correction')
            record.update(nut_yaw_directly_measured=True,
                grasp_orientation_source='CURRENT_RGBD_NUT_EXTERIOR_PHASE_WITH_ROBOT_MOTION_AND_REOBSERVATION',
                world_from_body_palm_five_dof=body.tolist(),
                target_world_from_hand=corrected_target.tolist())
        ...
        if error > tolerance:
            raise RuntimeError("the fresh palm pose leaves insufficient clearance for the planned nut grip")
        if visual_phase_active:
            record['visual_nut_phase_alignment'].update(completed=True,
                verified_position_error_m=error,original_position_tolerance_m=tolerance)
        ...
        if root_preload:
            ...
            # Use the same finite motor inverse as in loaded rotation. A small
            # decrement of a position reference is not evidence of reverse motion.
            from te_worm_drive import finger_force_motor_targets
            preload_initial_moments=None
            def regulate_root_moments(reference,measured,encoder):
                preload_filtered=root_observer.filter_moments(measured,stepper.step_index,dt,.05)
                targets,rows=finger_force_motor_targets(mechanism,encoder[8:],q,reference,preload_filtered,dt,
                    stiffness,float(root_preload['target_relaxation_time_constant_s']),
                    min(float(dynamic['finger_maximum_speed_rad_s']),float(root_preload['maximum_target_speed_rad_s'])),
                    .02,lower,upper)
                record['last_preload_motor_command']=rows
                record['preload_motor_interface']='SHARED_FINITE_FRICTION_AWARE_INPUT_VELOCITY'
                return targets
            def current_preload_readiness(reference):
                from te_nut_motion import preload_readiness
                window=max(2,round(float(ready_settings['window_s'])/dt))
                recent=[r['base_bridge_external_moment_nm'] for r in command_rows[-window:]
                        if 'base_bridge_external_moment_nm' in r]
                ...
                return preload_readiness(recent,reference,margins,
            ...
            for index in range(round(duration/dt)):
                ...
                if preload_initial_moments is None:
                    preload_initial_moments=measured.copy()
                    record['preload_initial_measured_moments_nm']=measured.tolist()
                blend=control.minimum_jerk_blend(min(1.,(index+1)*dt/ramp))
                reference=preload_initial_moments+blend*(desired-preload_initial_moments)
                q=regulate_root_moments(reference,measured,encoder)
            ...
            if refinement:
                ...
                for index in range(round(duration/dt)):
                    ...
                    if (ready_settings and ready_settings.get('finish_refinement_when_ready',False)
                            and (index+1)*dt>=ramp+float(ready_settings['window_s'])
                            and (index+1)%max(1,round(.05/dt))==0):
                        readiness=current_preload_readiness(desired)
                        if readiness['ready']:
                            record['refinement_early_finish']={
                                'elapsed_s':(index+1)*dt,'maximum_duration_s':duration,
                                'reference_ramp_finished':True,'readiness':readiness}
                            break
                record["root_moment_preload"]={**initial_stage,"targets_nm":desired.tolist(),
                    ...
                    "executed_refinement_duration_s":(index+1)*dt,
                    "final_measured_external_moment_nm":measured.tolist(),
                    "relative_reference_error":((measured-desired)/desired).tolist(),
        ...
        runtime['nut_grasp_yaw_bias_deg']=desired_yaw_bias
        record.update(completed=True, stage="NUT_GRIP_CONTROLLER_FINISHED_REQUIRES_PHYSICAL_EVALUATION",
                      ...
                      robot_sensor_history_file=str((output/'joint_ft_samples.json.gz').resolve()))
