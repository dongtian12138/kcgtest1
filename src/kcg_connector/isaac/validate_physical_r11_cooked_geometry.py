...
"""Fail closed on PhysX-cooked geometry drift in the physical-r11 asset.
...
PASS_BANNER = "ISAAC PHYSICAL R11 COOKED GEOMETRY PASSED"
FAIL_BANNER = "ISAAC PHYSICAL R11 COOKED GEOMETRY FAILED"
...
def _run(
    model_contract: Path | str | None = None,
    ...
    *,
    ...
    contract_path = Path(model_contract or DEFAULT_CONTRACT_PATH).expanduser().resolve()
    if contract_path.name == "d38999_keyed_v3_physical_model_contract_r12_v1.yaml":
        ...
        if authorized_model is not None:
            if candidate_index != 2:
                raise ValueError("local R12 cooked gate requires candidate index 2")
            model = authorized_model
        else:
            model = load_r12_physical_model_contract(contract_path)
            if candidate_index is not None:
                model = candidate_model(model, candidate_index)
    else:
        if candidate_index is not None:
            raise ValueError("candidate index is available only for r12")
        model = load_physical_model_contract(contract_path)
    ...
    if stage is None:
        raise RuntimeError("authorized physical asset could not be opened")
    ...
    if len(mesh_paths) != expected_mesh_count:
        raise RuntimeError("trusted mesh inventory does not close")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    status = 1
    try:
        report = _run(arguments.model_contract, arguments.candidate_index)
