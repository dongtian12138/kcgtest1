"""Static wrist RGB-D layout interface for the D38999 multilayer model.

This module maps the already frozen V5 wrist-camera definition onto the
current keyed-v3 robot and visual representation paths.  The wrist camera is
layout-evidence-only: it is not a shadow-inference or controller input, and
this module performs no USD write, rendering, or object/contact truth read.
"""
...
SCHEMA_VERSION = "kcg_d38999_multilayer_wrist_camera_interface_v1"
WRIST_MOUNT_CONTRACT = "SIM_VISUAL_MOUNT_CANDIDATE_FIXED_T_HC_V5"
...
def _verified_sources(root: Path) -> tuple[dict[str, str], ...]:
    ...
    for relative, expected in FROZEN_SOURCES.items():
        path = root / relative
        if not path.is_file():
            raise ValueError(f"frozen wrist source missing: {relative}")
        actual = _sha256(path)
        if actual != expected:
            raise ValueError(f"frozen wrist source hash mismatch: {relative}")
        rows.append({"path": relative, "sha256": actual})
    return tuple(rows)


def build_multilayer_wrist_camera_interface(
    repository_root: str | Path,
) -> dict[str, Any]:
    """Build a hash-bound, offline-only wrist camera interface manifest."""
    ...
    if not isinstance(assets, Mapping) or not isinstance(rig, Mapping):
        raise ValueError("camera assets or camera_rig missing")
    wrist = rig.get("wrist")
    if not isinstance(wrist, Mapping):
        raise ValueError("wrist camera definition missing")
    expected_parent = ROBOT_STAGE_ROOT + HANDBASE_SUFFIX
    if (
        rig.get("mount_contract") != WRIST_MOUNT_CONTRACT
        or rig.get("parent_frame") != "handbase_link"
        or assets.get("handbase_prim") != expected_parent
        or wrist.get("prim_suffix") != WRIST_PRIM_SUFFIX
        or wrist.get("role") != "WRIST_LAYOUT_EVIDENCE_ONLY"
    ):
        raise ValueError("frozen wrist mount identity changed")
    expected_t_hc = [
    ...
    if (
        wrist.get("v5_eye_handbase_m") != [-0.150, 0.0, 0.060]
        or wrist.get("v5_target_handbase_m") != [-0.090, 0.0, 0.480]
        or wrist.get("T_HC_cv") != expected_t_hc
    ):
        raise ValueError("frozen wrist V5 pose changed")
    if (
        ...
        raise ValueError("frozen wrist optics or output channels changed")
    ...
    if not all(
        ...
        for value in (capture, truth, authorization)
    ):
        raise ValueError("wrist capture, truth, or authorization boundary missing")
    if capture.get("wrist_shadow_allowed") is not False:
        raise ValueError("wrist shadow inference must remain forbidden")
    if truth.get("inference_channels_exactly") != [
        "rgb",
        "distance_to_image_plane",
    ]:
        raise ValueError("wrist inference channels changed")
    ...
    if not required_forbidden.issubset(forbidden):
        raise ValueError("wrist truth firewall weakened")
    capability_flags = (
        "control_authorized",
    ...
    if any(authorization.get(key) is not False for key in capability_flags):
        raise ValueError("wrist authorization must remain false")
    if (
        authorization.get("simulation_only") is not True
        or authorization.get("viewpoint_human_review_required") is not True
    ):
        raise ValueError("wrist evidence review boundary changed")

    return {
        "schema_version": SCHEMA_VERSION,
        "status": "OFFLINE_PASS",
        "interface_role": "WRIST_LAYOUT_EVIDENCE_ONLY",
        "mount_contract": WRIST_MOUNT_CONTRACT,
        "robot_asset": (
            "artifacts/kcg_connector/isaac/robot/"
            "handarm_keyed_v3_physical_r7/handarm.usda"
        ),
        ...
        "camera_prim": expected_parent + WRIST_PRIM_SUFFIX,
        "v5_eye_handbase_m": wrist["v5_eye_handbase_m"],
        "v5_target_handbase_m": wrist["v5_target_handbase_m"],
        "T_HC_cv": wrist["T_HC_cv"],
