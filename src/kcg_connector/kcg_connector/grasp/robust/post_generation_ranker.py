"""Post-generation, rank-only evaluation for frozen CARTS V9 candidates.
...
V9/model lineage, and evaluates every unique statically accepted candidate
once with one common immutable Sobol design.  Collision and task-wrench
evaluation are independent calls, so an unresolved collision certificate
cannot erase the diagnostic wrench result.
...
``NOT_CERTIFIABLE``.  Therefore this V1 rank-only bridge may report a
diagnostic order when a separately typed complete-clearance certificate is
available, but it never creates a formal winner.  ``selected_candidate`` must
remain ``None`` until a future evaluator actually consumes all calibrated
non-friction uncertainty bounds and a complete approach/closure/lift
...
METHOD_ID = "CARTS_POST_GENERATION_COMMON_QMC_LEXICOGRAPHIC_RANKER_V1"
...
class RankCandidateRecord:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise PostGenerationRankingError(
                "candidate rank record violates the V1 fail-closed contract"
            )
...
class PostGenerationRankResult:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise PostGenerationRankingError(
                "post-generation result violates its immutable V1 contract"
            )
...
class PostGenerationRankOnlyPipeline:
    ...
    def _validate_generation(
        ...
        if result.accepted_policies:
            raise PostGenerationRankingError(POLICY_AWARE_RANKING_GUARD)
        if any(
        ...
        return snapshots, tuple(bound_rows), _sha256(result_document)
    ...
    def _diagnostic_rank(
        records: Sequence[RankCandidateRecord],
    ) -> tuple[tuple[RankCandidateRecord, ...], tuple[str, ...]]:
        rows = tuple(records)
        eligible = tuple(
            index
    ...
    def evaluate(
        ...
        """Evaluate each unique static V9 acceptance once per stage."""

        snapshots, accepted_rows, generation_sha256 = (
        ...
        if unresolved:
            selection_blockers.add(
                "UNRESOLVED_ACCEPTED_CANDIDATES_PREVENT_FINITE_DESIGN_WINNER"
            )
        result_document = {
            "method_id": METHOD_ID,
