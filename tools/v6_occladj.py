#!/usr/bin/env python3
"""Test occluder-adjusted support on v6 data: legacy mating + shell25j occluders."""
import json
import math
import sys
...
def main() -> int:
    ...
    configs = [
        ("joint_cadoccl_ms17", dict(plug_cad=legacy_plug, receptacle_cad=legacy_rec,
        ...
        ("palm_cadoccl_ms17", dict(plug_cad=legacy_plug, receptacle_cad=legacy_rec,
             plug_occluder_cad=shell.plug_occluders,
             occlusion_policy="ignore_foreground_and_cad_occluder",
             edge_policy="depth_gated",
             optimizer_variant="multistart_physical_jacobian", multistart_count=17)),
    ...
    for label, kwargs in configs:
        views = [palm] if label.startswith("palm") else [palm, wrist]
        ...
        for hyp in result["c2"]["hypotheses"]:
            ...
            for d in hyp.get("plug_support_diagnostics") or []:
                print(f"    {d['view_id']}: vis={d['visible_depth_support_fraction']:.3f} "
                      ...
                      f"cad_occ={d['cad_occluder_fraction']:.3f} "
                      f"missing={d['missing_surface_fraction']:.3f} dgate={d['depth_gated_edge_support_fraction']:.3f}")
